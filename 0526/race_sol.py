...
def bfs(x, y, cost, direct):
    queue = deque([(x, y, cost, direct)]) # 좌표, 비용, 방향
    check = [[0 for _ in range(N)] for _ in range(N)] # 방문 체크
    check[x][y] = 1 # 1원인 경우 없으니까 단지 조건에 걸리지 않게 체크
    while queue:
        ...
        for i, j, d in (0, 1, 'R'), (1, 0, 'D'), (0, -1, 'L'), (-1, 0, 'U'):
            ...
            if new_x < 0 or new_y < 0 or new_x >= N or new_y >= N:
                continue
            if not new_board[new_x][new_y]: # 벽이 없으면
                # 처음 방문이거나 최소값이
...
# 도착하는 시간을 전부 체크 (최솟값만 빼놓는게 아니라)
# 새 경로마다 전부 체크하면 무조건 시간초관데 어떻게 ?
