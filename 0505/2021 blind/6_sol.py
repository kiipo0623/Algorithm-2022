    ...
    while q:
        ...
        for dir in range(4):
            ...
            while True:
                nx = cur[0] + dx[dir]*en
                ny = cur[1] + dy[dir]*en
                if OOB(nx+dx[dir], ny+dy[dir]) or (en != 0 and board[nx][ny]):
                    break
                en += 1
...
def solution(board, r, c):
    occur = [[] for i in range(7)] # 숫자에 해당하는 좌표 위치
    brute = [] # 숫자들
    ...
    for p in itertools.permutations(brute):
        ...
        for i in range(1, n):
            d[i][0] = min(d[i - 1][0] + dist(myboard, occur[p[i - 1]][1], occur[p[i]][0]),
                          d[i - 1][1] + dist(myboard, occur[p[i - 1]][0], occur[p[i]][0])) + dist(myboard,
                                                                                                  occur[p[i]][0],
                                                                                                  occur[p[i]][1])
            d[i][1] = min(d[i - 1][0] + dist(myboard, occur[p[i - 1]][1], occur[p[i]][1]),
                          d[i - 1][1] + dist(myboard, occur[p[i - 1]][0], occur[p[i]][1])) + dist(myboard,
                                                                                                  occur[p[i]][1],
                                                                                                  occur[p[i]][0])
